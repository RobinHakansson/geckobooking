...
public class PBKDF2
{
    ...
    public byte[] GetBytes(string password, string salt, int iterations, int howManyBytes)
    {
        return GetBytes(System.Text.Encoding.UTF8.GetBytes(password), System.Text.Encoding.UTF8.GetBytes(salt), iterations, howManyBytes);
    }
    ...
    static void _xorByteArray(byte[] src, int srcOffset, int cb, byte[] dest, int destOffset)
    {
        ...
        while (srcOffset != end)
        {
            dest[destOffset] ^= src[srcOffset]; ++srcOffset; ++destOffset;
        }
    }
}